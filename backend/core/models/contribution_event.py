...
SPRINT-5B ADDITIONS:
- contributor_fingerprint: Immutable, non-PII identifier for evaluation-time identity
- This field survives account deletion and preserves contributor independence semantics
- Evaluation logic MUST use contributor_fingerprint, NOT contributor_id

IDENTITY SEPARATION (PH-5 Compliance):
- contributor_fingerprint: For evaluation (immutable, non-nullable, event-level)
- contributor: For ownership (nullable FK, removable via account deletion)
"""
...
class ContributionEvent(ImmutableModel):
    ...
    contributor = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,  # Sprint-5B: Allow account deletion
        null=True,  # Sprint-5B: Nullable after account deletion
        blank=True,
    ...
    # === Contributor Identity for Evaluation (Sprint-5B) ===
    # NOTE: This is an INTERNAL evaluation identifier. NEVER expose via public API.
    # In Phase-2, fingerprints may represent non-user sources (GTFS imports, OSM sync).
    # The design intentionally decouples "contributor identity" from "user account".

    contributor_fingerprint = models.UUIDField(
        db_index=True,
        editable=False,
        help_text=(
    ...
    class Meta:
        ...
        indexes = [
            ...
            # Sprint-5B: Index for evaluation queries using fingerprint
            models.Index(fields=["contributor_fingerprint", "submitted_at"]),
        ]

    def __str__(self):
        # Sprint-5B: Handle nullable contributor after account deletion
        contributor_display = (
            self.contributor.username if self.contributor else "[deleted]"
        )
        return (
            f"{self.get_contribution_type_display()} "
            f"by {contributor_display} "
    ...
    def clean(self):
        ...
        - contributor_fingerprint must be explicitly provided (Sprint-5B)
        ...
        # Sprint-5B: contributor_fingerprint is REQUIRED and must be EXPLICITLY provided
        # This is a critical invariant for evaluation independence (INV-I1, INV-I2)
        # It must NEVER be auto-derived - callers must explicitly set it
        if self.contributor_fingerprint is None:
            raise ValidationError(
                {
                    "contributor_fingerprint": (
                        "contributor_fingerprint is required and must be explicitly "
                        "provided at creation time. Set it to contributor.id."
                    )
                }
            )
    ...
    def create_or_get_idempotent(cls, client_generated_id, **kwargs):
        ...
        Sprint-5B: contributor_fingerprint MUST be explicitly provided.
        It is NOT auto-derived. Callers must set contributor_fingerprint=contributor.id.

        Args:
            client_generated_id: UUID generated by client
            **kwargs: Other fields for ContributionEvent
                      Must include 'contributor_fingerprint' for new events
        ...
        Example:
            event, created = ContributionEvent.create_or_get_idempotent(
                ...
                contributor=user,
                contributor_fingerprint=user.id,  # REQUIRED - explicit
